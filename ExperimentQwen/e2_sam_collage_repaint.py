...
def generate_setup(pipe,args,setup_dir):
 ...
 for index,(name,path) in enumerate(tqdm(discover_sketches(args.sketch_dir),desc='Generating E2 references',unit='object'),1):
  ...
  image=infer(pipe,[sketch],prompt,args,args.object_seed);target=objects_dir/f'{index:02d}_{name}.png';image.save(target)
  objects.append({'name':name,'sketch':str(path),'image':str(target),'seed':args.object_seed})
...
def main():
 ...
 p.add_argument('--mask_backend',choices=('auto','sam2','difference'),default='auto');p.add_argument('--sam_model_id',default='facebook/sam2-hiera-small');p.add_argument('--sam_device',default='cpu');p.add_argument('--device',default='cuda');p.add_argument('--width',type=int,default=1024);p.add_argument('--height',type=int,default=1024);p.add_argument('--steps',type=int,default=8);p.add_argument('--seed',type=int,default=42);p.add_argument('--object_seed',type=int,default=1337,help='Fixed seed reused for every sketch-to-object generation');p.add_argument('--true_cfg_scale',type=float,default=1);p.add_argument('--negative_prompt',default=' ')
