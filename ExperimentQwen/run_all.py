...
def parse_args():
    ...
    parser.add_argument("--e12_case_ids", type=int, nargs="+", help="Subset of prompt-suite cases for E12")
    parser.add_argument("--e12_max_objects", type=int, choices=(1, 2, 3), help="Limit objects per E12 case")
    ...
    parser.add_argument("--e13_case_ids", type=int, nargs="+", help="Subset of prompt-suite cases for E13")
    ...
    parser.add_argument("--e14_variants", nargs="+", choices=("native", "asymmetric", "difference", "correlation", "sinkhorn"), default=["native", "asymmetric", "difference", "correlation", "sinkhorn"], help="E14 matched conditioning and attention ablations")
...
def main():
    ...
    for experiment in selected:
        ...
        if experiment["id"] == "e12":
            command.extend([
                "--reference_area_ratio", str(args.e12_reference_ratio),
            ...
            if args.e12_max_objects is not None:
                command.extend(["--max_objects", str(args.e12_max_objects)])
        if experiment["id"] == "e13":
            command.extend(["--metric_device", args.e13_metric_device])
            if args.e13_no_resume:
                command.append("--no-resume")
            if args.e13_case_ids:
