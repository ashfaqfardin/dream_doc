"""
E10: Scene-Aware Adaptive Placement + Identity-Preserving Incremental Editing
=================================================================
...
Core idea
---------
E9 could collapse to the image centre because one unconstrained proposal was
used to define the placement mask. E10 replaces that single proposal with a
MULTI-HYPOTHESIS scene-aware search. For every new object it generates up to
four placement/pose hypotheses (free, left, right, back/side), derives a mask
for each, scores them for object identity, localised change, overlap with
previous objects, border clipping, and exact-centre collapse, then selects the
best hypothesis. The winning proposal seed is reused in the final pass so the
selected viewpoint/placement is more stable.

The reference object is NEVER pre-warped. FLUX may adapt viewpoint, orientation,
apparent scale, foreshortening and perspective, while identity-defining geometry,
colour, materials and distinctive parts are explicitly preserved.

Each incremental edit still uses TWO stages:
...
pixel difference can include unrelated proposal artefacts. By default E10 uses:
...
E10 validates that the context section divides evenly between context images.
    ...
    def __init__(
        ...
        for i in blocks:
            ...
            if "IPAdapter" in orig.__class__.__name__:
                raise RuntimeError(f"Block {i} uses IP-Adapter processor; E10 does not wrap it.")
...
def object_affordance(new_name: str) -> str:
    """Light semantic prior: how this object normally relates to a room.

    This does NOT fix coordinates. It gives FLUX an affordance while candidate
    prompts explore different scene regions. The reference image remains clean.
    """
    priors = {
        ...
        "plant": "standing naturally on the floor near a wall/corner or on a suitable surface if the reference scale implies a small plant",
        "backpack": "resting naturally near furniture, against a wall, or on a suitable support rather than floating",
    }
    return priors.get(new_name, "placed in a physically supported, semantically appropriate location")


def placement_candidate_hints(new_name: str) -> List[Tuple[str, str]]:
    """Candidate scene regions. FLUX still decides exact coordinates/pose.

    We intentionally include a free candidate, but do not allow a single
    centre-biased proposal to define the mask as E9 did.
    """
    afford = object_affordance(new_name)
    return [
        ("free", f"Choose the most natural location anywhere in the room; the object should be {afford}."),
        ("left", f"Find a natural location in the LEFT half of the room; the object should be {afford}. Avoid the exact image centre."),
        ("right", f"Find a natural location in the RIGHT half of the room; the object should be {afford}. Avoid the exact image centre."),
        ("back_or_side", f"Prefer a plausible BACK or SIDE area of the room with physical support; the object should be {afford}. Avoid the exact image centre unless genuinely necessary."),
    ]
...
def build_proposal_prompt(
    ...
    prompt = (
        f"Add the {new_name} exactly once. {placement_hint} "
        f"Use the reference only to preserve the {new_name}'s identity-defining structure, exact color palette, materials, texture, and distinctive design. "
        "Adapt its apparent size to the scene. It may rotate, turn, change viewpoint, foreshorten, and undergo normal perspective projection so it sits naturally in the room. "
        "For rigid objects, do not bend, melt, redesign, or change their structural geometry. Preserve characteristic proportions and parts even when the camera view changes. "
        "Respect floor/surface contact, room depth, perspective, occlusion, and realistic orientation. "
    )
    if previous_names:
        prompt += (
...
def build_final_prompt(
    ...
    where = f" around the {pos}" if pos else " in the selected room region"
    prompt = (
        f"Add the {new_name} exactly once{where}. {selected_hint} "
        f"Match the reference object's identity: preserve its exact colors, materials, texture, distinctive parts, and identity-defining geometry. "
        "Adapt only viewpoint, orientation, apparent scale, foreshortening, and perspective as required by the room and camera. "
        "Do not copy the reference as a flat front-facing sticker. Render it as the same object viewed naturally in this scene. "
        "Do not bend, melt, redesign, or replace characteristic structural parts. Make physical contact with the appropriate floor or supporting surface and respect scene depth and occlusion. "
    )
    if previous_names:
        prompt += (
...
def score_proposal_candidate(
    ...
    """Score placement without forcing a hand-authored coordinate.

    Rewards local, identity-bearing edits; penalises overlap, border clipping and
    E9's repeated exact-centre failure. Identity is measured on the proposed crop.
    """
    mask = placement.soft_mask
    box = mask_bbox(placement.hard_mask)
    identity = 0.0
    if box is not None:
        try:
            crop = proposal.crop(expand_bbox(box, proposal.size, frac=0.08))
            identity = float(compute_dino(new_ref, crop, device))
        except Exception as exc:
            warnings.warn(f"Candidate DINO score failed: {exc}")

    # Difference localisation: desired change should be concentrated inside mask.
    ...
    # Only a mild penalty for the exact centre; natural central placement remains possible.
    exact_center_penalty = max(0.0, 1.0 - dist_center / max(args.center_deadzone, 1e-4))

    score = (
        args.score_identity_weight * identity
    ...
    comp = {
        "identity_dino": identity, "locality": locality, "previous_overlap": overlap,
        "border_fraction": border, "center_distance": dist_center,
        "exact_center_penalty": exact_center_penalty, "centroid_x": cx, "centroid_y": cy,
    }
...
def run_proposal_pass(
    ...
    prompt = build_proposal_prompt(new_name, previous_names, placement_hint=placement_hint)
    accumulator = PlacementAccumulator(
        n_target_tokens=n_target_tokens,
...
def run_final_pass(
    ...
    prompt = build_final_prompt(new_name, active_names[:-1], placement_mask, selected_hint=selected_hint)
...
def parse_args():
    ...
    p.add_argument("--out_dir", default="results/e10_scene_aware_adaptive_placement")
    ...
    # E10 multi-hypothesis scene-aware placement selection
    p.add_argument("--placement_candidates", type=int, default=4, choices=[1,2,3,4],
                   help="Number of model placement hypotheses evaluated per object")
    p.add_argument("--score_identity_weight", type=float, default=1.00)
    p.add_argument("--score_locality_weight", type=float, default=0.65)
    ...
    p.add_argument("--score_center_weight", type=float, default=0.30,
                   help="Mild anti-collapse penalty only very near exact image centre")
    p.add_argument("--center_deadzone", type=float, default=0.18,
                   help="Normalised radius in which exact-centre collapse is penalised")
...
def main():
    ...
    with open(os.path.join(args.out_dir, "config_e10.json"), "w", encoding="utf-8") as f:
    ...
    print("\n=== E10: SCENE-AWARE ADAPTIVE PLACEMENT ===")
    ...
    print("placement strategy  : multi-hypothesis + identity/locality/overlap scoring")
    ...
    for seed in seeds:
        ...
        for step, new_name in enumerate(available, start=1):
            ...
            print("  Pass A: multi-hypothesis scene-aware placement search")
            candidates: List[ProposalCandidate] = []
            candidate_root = os.path.join(step_dir, "placement_candidates")
            os.makedirs(candidate_root, exist_ok=True)
            hints = placement_candidate_hints(new_name)[:args.placement_candidates]

            for ci, (cand_name, cand_hint) in enumerate(hints):
                cand_seed = int(step_seed + ci * 97)
                cand_dir = os.path.join(candidate_root, f"{ci+1:02d}_{cand_name}")
                os.makedirs(cand_dir, exist_ok=True)
                print(f"    candidate {ci+1}/{len(hints)}: {cand_name}")
                cand_prop, cand_attn, cand_prompt = run_proposal_pass(
                    pipe=pipe, scene=current_scene, new_name=new_name,
                    previous_names=previous_names, obj_refs=obj_refs, ref_masks=ref_masks,
                    n_target_tokens=n_target_tokens, localization_blocks=localization_blocks,
                    args=args, step_seed=cand_seed, placement_hint=cand_hint,
                )
                cand_diff = proposal_difference_map(
                    previous=current_scene, proposal=cand_prop,
                    grid_size=(target_gw, target_gh), blur_px=args.difference_blur_px,
                )
                cand_place = derive_placement_mask(
                    attention_map=cand_attn, difference_map=cand_diff,
                    output_size=current_scene.size, source=args.mask_source,
                    attention_weight=args.attention_weight, mask_quantile=args.mask_quantile,
                    min_mask_frac=args.min_placement_frac, max_mask_frac=args.max_placement_frac,
                    score_blur_tokens=args.score_blur_tokens, close_tokens=args.close_tokens,
                    dilate_tokens=args.dilate_tokens, feather_px=args.mask_feather_px,
                )
                cand_score, cand_components = score_proposal_candidate(
                    scene=current_scene, proposal=cand_prop, new_ref=obj_refs[new_name],
                    placement=cand_place, previous_masks=[placement_masks[n] for n in previous_names if n in placement_masks],
                    device=args.device, args=args,
                )
                cand_prop.save(os.path.join(cand_dir, "proposal.png"))
                cand_place.soft_mask.save(os.path.join(cand_dir, "placement_mask_soft.png"))
                make_overlay(current_scene, cand_place.soft_mask).save(os.path.join(cand_dir, "overlay.png"))
                with open(os.path.join(cand_dir, "score.json"), "w") as cf:
                    json.dump({"name": cand_name, "hint": cand_hint, "score": cand_score,
                               "components": cand_components, "prompt": cand_prompt}, cf, indent=2)
                print(f"      score={cand_score:.4f}  location={position_phrase(cand_place.soft_mask)} "
                      f"DINO={cand_components['identity_dino']:.3f} overlap={cand_components['previous_overlap']:.3f}")
                candidates.append(ProposalCandidate(
                    name=cand_name, hint=cand_hint, seed=cand_seed, proposal=cand_prop, attention_map=cand_attn,
                    difference_map=cand_diff, placement=cand_place, score=cand_score,
                    components=cand_components, prompt=cand_prompt))

            selected = max(candidates, key=lambda c: c.score)
            proposal = selected.proposal
            attention_map = selected.attention_map
            difference_map = selected.difference_map
            placement = selected.placement
            proposal_prompt = selected.prompt
            proposal.save(os.path.join(step_dir, "proposal.png"))
            with open(os.path.join(step_dir, "placement_selection.json"), "w") as sf:
                json.dump({
                    "selected": selected.name, "selected_hint": selected.hint,
                    "selected_seed": selected.seed, "selected_score": selected.score, "selected_components": selected.components,
                    "all_candidates": [{"name": c.name, "seed": c.seed, "score": c.score, "components": c.components} for c in candidates]
                }, sf, indent=2)
            print(f"  selected placement: {selected.name} score={selected.score:.4f} "
            ...
            record = {
                ...
                "selected_placement_candidate": selected.name,
                "selected_placement_hint": selected.hint,
                "selected_placement_seed": selected.seed,
                "selected_placement_score": selected.score,
                "selected_placement_components": selected.components,
