...
def final_composite(current,raw,pre_mask,job,detector,sam,args,directory):
 ...
 union=Image.fromarray(union.astype(np.uint8),'L')
...
def parse_args():
 ...
 p.add_argument('--inpaint_steps',type=int,default=28);p.add_argument('--inpaint_guidance_scale',type=float,default=2.5);p.add_argument('--inpaint_strength',type=float,default=.88);p.add_argument('--detection_threshold',type=float,default=.2);p.add_argument('--diff_blur_px',type=float,default=3);p.add_argument('--min_object_area',type=float,default=.003);p.add_argument('--max_object_area',type=float,default=.35);p.add_argument('--mask_dilate_px',type=int,default=16);p.add_argument('--mask_feather_px',type=float,default=6)
 p.add_argument('--final_composite_mode',choices=['post_sam','raw'],default='post_sam',help='Use post-inpaint silhouette protection, or accept the complete raw result')
