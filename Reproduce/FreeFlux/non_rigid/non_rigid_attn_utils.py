...
class FluxAttnProcessor2_0_Non_Rigid:
    ...
    def __init__(self, start_step=4, start_layer=0, layer_idx=None, step_idx=None, total_layers=57, total_steps=50):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("FluxAttnProcessor2_0_Non_Rigid requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        ...
        print("Non-Rigid Editing at denoising steps: ", self.step_idx)
        print("Non-Rigid Editing at U-Net layers: ", self.layer_idx)
...
def register_non_rigid_attention_control(model, **attn_args):
    attn_procs = FluxAttnProcessor2_0_Non_Rigid(**attn_args)
    model.transformer.set_attn_processor(attn_procs)
    print(f"Model {model.transformer.__class__.__name__} is registered attention processor: FluxAttnProcessor2_0_Non_Rigid")
