...
import cv2
import types
from ..rewrite_schedule import step_forward
...
class FluxPipeline(
    ...
    def remove_small_white_regions(self, binary_image: np.ndarray) -> np.ndarray:
    ...
    def min_max_normalize(self, attn_map):
        min_val = torch.min(attn_map)
        max_val = torch.max(attn_map)
        return (attn_map - min_val) / (max_val - min_val + 1e-6)

    def derive_add_mask_from_attn(self, global_store, add_idx_list, target_step=7, mask_threshold=0.3, height=64, width=64):
        ...
        avg_attn = self.min_max_normalize(torch.mean(torch.stack(target_attn_list), dim=0))
        derived_mask = 255 * (avg_attn > mask_threshold).int().numpy().astype(np.uint8)
        filtered_image = self.remove_small_white_regions(derived_mask)
        indices = [x + 512 for x in np.where(filtered_image.flatten() == 1)[0]]
        return indices
    ...
    def __call__(
        ...
        max_sequence_length: int = 512,
        subject_idx_list: List = None,
        derive_step: int = 7,
        derive_idx_list: List = None,
        ...
        self.scheduler.step_forward = types.MethodType(step_forward, self.scheduler)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if (subject_idx_list is not None) and (i == derive_step):
                    global_store = self.transformer.single_transformer_blocks[1].attn.processor.global_store
                    derive_idx_list = self.derive_add_mask_from_attn(global_store, subject_idx_list, target_step=derive_step, mask_threshold=0.3, height=height//16, width=width//16)
                    return derive_idx_list
