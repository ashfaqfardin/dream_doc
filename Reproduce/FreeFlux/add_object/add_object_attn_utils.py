...
class FluxAttnProcessor2_0_Reasoning:
    ...
    def masactrl_forward(
        ...
        kc_tgt_modified = kc_tgt.clone()
        vc_tgt_modified = vc_tgt.clone()

        kc_tgt_modified[:,:,512:,:] = kc_src[:,:,512:,:]
        vc_tgt_modified[:,:,512:,:] = vc_src[:,:,512:,:]

        k_rearranged = torch.cat([kc_src, kc_tgt_modified])
    ...
    def attn_forward(
        ...
        q_src, q_tgt = query.chunk(2)
        k_src, k_tgt = key.chunk(2)
...
class FluxAttnProcessor2_0_Add_Object:
    """Attention processor used typically in processing the SD3-like self-attention projections."""

    def __init__(self, start_step=4, start_layer=0, layer_idx=None, step_idx=None, total_layers=57, total_steps=50):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("FluxAttnProcessor2_0_Add_Object requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        ...
        self.layer_idx = layer_idx if layer_idx is not None else list(range(start_layer, self.total_layers))
        self.step_idx = step_idx if step_idx is not None else list(range(start_step, total_steps))
        print("Add object control at denoising steps: ", self.step_idx)
        print("Add object control at U-Net layers: ", self.layer_idx)
    ...
    def masactrl_forward(
        ...
        kc_tgt_modified[:,:,derive_idx_list,:] = kc_tgt[:,:,derive_idx_list,:]
        vc_tgt_modified[:,:,derive_idx_list,:] = vc_tgt[:,:,derive_idx_list,:]
    ...
    def attn_forward(
        ...
        if self.cur_step in self.step_idx and cur_transformer_layer in self.layer_idx:
            query, key, value = self.masactrl_forward(query, key, value, derive_idx_list)
...
def register_add_object_attention_control(model, **attn_args):
    attn_procs = FluxAttnProcessor2_0_Add_Object(**attn_args)
    model.transformer.set_attn_processor(attn_procs)
    print(f"Model {model.transformer.__class__.__name__} is registered attention processor: FluxAttnProcessor2_0_Add_Object")
